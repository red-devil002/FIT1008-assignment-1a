...
POSSIBLE_ADT_TYPES = Union[
    ArrayR, ArraySet, BitVectorSet, CircularQueue,
    LinkedList, CircularQueue, ArrayStack
]
...
def take_out_from_adt(adt: POSSIBLE_ADT_TYPES) -> ArrayR[T] | None:
    ...
    adt_type = type(adt)
    if adt_type in [CircularQueue]:
        for index in range(len(adt)):
            output[index] = adt.serve()
            adt.append(output[index])

    elif adt_type == ArrayStack:
        temp_stack = ArrayStack(len(adt))
        for index in range(len(adt)):
...
def test_stack() -> None:
    ...
    """
    stack: ArrayStack = ArrayStack(20)
    for i in range(1, 11):
        stack.push(i)
